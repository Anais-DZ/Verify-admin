...
class AdminController extends GenericController
{
    ...
    public function registerWaste(): string
    {
        ...
        if (isset($_POST['submitAddWaste'])) {
            ...
            $nameWaste = cleanForDB($_POST['nameWaste']);
            $idContainer = cleanForDB($_POST['idContainer']);
    ...
    public function upDateWaste(): string
    {
        ...
        if (isset($_POST['submitUpdateWaste'])) {
            ...
            $idWaste = cleanForDB($_POST['idWasteUpDate']);
            $nameWaste = cleanForDB($_POST['nameWasteUpDate']);
            $idContainer = cleanForDB($_POST['newIdContainer']);
    ...
    public function removeWasteById(): string
    {
        ...
        if (isset($_POST['submitDeleteById'])) {
            ...
            //3_ Nettoyer les données
            $idWaste = cleanForDB($_POST['idWaste']);
    ...
    public function removeWasteByName(): string
    {
        ...
        if (isset($_POST['submitDeleteWaste'])) {
            ...
            //3_ Nettoyer les données
            $nameWaste = cleanForDB($_POST['nameWasteToDelete']);
    ...
    public function readWaste(): string
    {
        ...
        foreach ($data as $waste) {
            ...
            $wastesTable .= "<tr>
                                <td>" . cleanForHTML($waste['name_waste']) . "</td>
                                <td>" . cleanForHTML($waste['id_waste']) . "</td>
                                <td class='containerColumn'>" . cleanForHTML($waste['type_container']) . "</td>
                                <td>" . cleanForHTML($waste['id_container']) . "</td>
                                <td>
                                    <form method='POST' action='adminWaste.php'>
                                        <input type='hidden' name='idWaste' value=" . cleanForHTML($waste['id_waste']) . ">
